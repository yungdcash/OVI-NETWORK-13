...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    console.log('Auth state changed:', { supabaseUser, supabaseLoading, session });
    if (!supabaseLoading) {
      if (supabaseUser) {
  ...
  }, [supabaseUser, supabaseLoading, session]);
  ...
  useEffect(() => {
    ...
    if (verified === 'true' || success === 'true' || type === 'signup') {
      console.log('Email verification detected, will redirect after auth');
  ...
  const createProfile = async (userId) => {
    try {
      ...
      const displayName = supabaseUser?.user_metadata?.full_name || supabaseUser?.user_metadata?.display_name || username;
